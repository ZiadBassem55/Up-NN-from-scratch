...
def init_parameters(): #لاني هستخدم الrandom في الweights فهنا هستخدم الseed علشان اقدر اعمل تكرار للنتائج
    r.seed(42)
    w1 = np.random.uniform(-0.5, 0.5, (input_neurons, hidden_neurons)) #w1 هنا هعمل الweights من الinput layer للhidden layer
    b1 = 0.5
    w2 = np.random.uniform(-0.5, 0.5, (hidden_neurons, output_neurons))#w2 هنا هعمل الweights من الhidden layer للoutput layer
    b2 = 0.7
    return w1, w2, b1, b2
